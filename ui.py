...
# 3. Configuration & API Key (server-side only — never exposed to users)
# On Hugging Face Spaces, secrets set in "Settings -> Variables and secrets"
# are injected directly into the environment, so os.getenv picks them up.
...
# 3.1 Fail loudly (to you, the admin) if the server-side key is missing.
# Users never see a key input box.
if not api_key:
    st.error("⚠️ GROQ_API_KEY is not configured on the server. Please contact the administrator.")
    st.stop()
